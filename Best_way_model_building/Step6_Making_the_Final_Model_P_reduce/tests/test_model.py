...
def test_model_param_count():
    ...
    assert total_params < 10000, f"Model has {total_params} parameters, which exceeds the limit of 10,000."
...
def test_model_contains_batchnorm_dropout():
    model = Net()
    
    # Check for presence of BatchNorm and Dropout layers
    has_batchnorm = False
    has_dropout = False
    ...
    assert has_batchnorm, "Model should contain at least one BatchNormalization layer"
    assert has_dropout, "Model should contain at least one Dropout layer"
    
    # Optionally check dropout rate (assuming you want dropout rate > 0.2)
    for module in model.modules():
        if isinstance(module, torch.nn.Dropout):
            assert module.p <= 0.2, f"Dropout rate should be <= 0.2, but got {module.p}"
